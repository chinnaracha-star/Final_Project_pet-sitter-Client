...
export const getSitterBookings = (query = '') => {
  if (!useMock()) return request<SitterBooking[]>(`/api/bookings/sitter?query=${encodeURIComponent(query)}`)
  const needle = query.trim().toLowerCase()
  return Promise.resolve(mockBookings.filter(b => !needle || b.owner.name.toLowerCase().includes(needle) || b.transactionNo?.toLowerCase().includes(needle) || b.pets.some(p => p.name.toLowerCase().includes(needle))))
}
...
export const updateBookingStatus = (id: number, status: 'waiting_service' | 'cancelled' | 'success') => {
  if (useMock()) { const booking = mockBookings.find(b => b.id === id) || mockBookings[0]; booking.status = status; return Promise.resolve(booking) }
