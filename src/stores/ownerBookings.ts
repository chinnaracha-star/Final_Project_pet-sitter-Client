...
const STORAGE_KEY = 'pet_sitter_owner_bookings'

const defaultBookings: OwnerBooking[] = [
  {
    id: 1,
...
function loadStoredBookings(): OwnerBooking[] {
  try {
...
export const useOwnerBookingsStore = defineStore('ownerBookings', () => {
  const bookings = ref<OwnerBooking[]>(loadStoredBookings())
  ...
  function addBooking(booking: OwnerBooking) {
  ...
  return { bookings, addBooking, addReview, addReport }
})
