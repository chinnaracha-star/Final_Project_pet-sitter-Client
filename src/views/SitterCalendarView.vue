...
const router=useRouter(), anchor=ref(new Date()), bookings=ref<SitterBooking[]>([]), query=ref(''), error=ref('')
...
async function load(){error.value='';try{bookings.value=await getSitterBookings(query.value,iso(days.value[0]),iso(days.value[6]))}catch(e){error.value=e instanceof Error?e.message:'Unable to load calendar'}}
...
<template><SitterPageShell><main class="page"><SitterDemoNotice /><div class="top"><h1>Calendar</h1><form @submit.prevent="load"><input v-model.trim="query" placeholder="Search..." aria-label="Search calendar"/></form></div><div class="tools"><button @click="today">Today</button><button @click="move(-1)">‹</button><b>{{days[0].toLocaleDateString('en-GB',{day:'numeric',month:'short'})}} – {{days[6].toLocaleDateString('en-GB',{day:'numeric',month:'short',year:'numeric'})}}</b><button @click="move(1)">›</button><label v-for="s in allStatuses" :key="s"><input v-model="selected" type="checkbox" :value="s"/> {{s.replaceAll('_',' ')}}</label></div><p v-if="error" class="error">{{error}}</p>
